...
public class NetCommand {
	...
	public final static Logger log = Logger.getLogger(NetCommand.class);
	...
	// 命令执行结果
	private String cmdRes = ConfigBean.RES_OK;
	...
	//				DefConf.INT_SIZE+ // 整个命令长度
					ConfigBean.INT_SIZE+ // version
							ConfigBean.INT_SIZE+ // cmdType
							ConfigBean.INT_SIZE+ // cmdId 长度
					this.cmdId.getBytes().length+ // cmdId
							ConfigBean.INT_SIZE+ // cmdCode 长度
					this.cmdCode.getBytes().length+ // cmdCode
							ConfigBean.INT_SIZE+ // cmdRes 长度
					this.cmdRes.getBytes().length+ // cmdRes
							ConfigBean.INT_SIZE+ // cmdResMsg 长度
					this.cmdResMsg.getBytes().length+ // cmdResMsg
							ConfigBean.INT_SIZE+ // 命令数据长度的长度
					dataLength; // 命令数据长度
			
			 // 版本 
	//		 ByteBuffer cmdBuffer = ByteBuffer.allocateDirect(4+4+4+4+this.cmdCode.getBytes().length+4+dataLength);
			 ByteBuffer cmdBuffer = ByteBuffer.allocateDirect(ConfigBean.INT_SIZE+totleCmdLength);
			...
			log.error("encodeCmd 错误:"+ this.getCmdCode());
